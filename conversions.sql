...
SELECT inv.project_id, inv.spec_id, inv.region_id, inv.cnt_invited, workd.cnt_working, (workd.cnt_working / inv.cnt_invited) conversion
FROM 
	(SELECT project_id, spec_id, region_id, COUNT(*) cnt_invited
FROM couriers 
WHERE created BETWEEN '2021,9,1' AND '2021,9,10'
GROUP BY project_id, spec_id) inv
JOIN 
	(SELECT project_id, spec_id, region_id, COUNT(*) cnt_working
	FROM couriers 
	WHERE id IN 
	...
	GROUP BY project_id, spec_id, region_id) workd
...
SELECT state_details, cnt, SUM(a.cnt) OVER() summa, cnt/(SUM(a.cnt) OVER()) conversion
FROM 
	(SELECT state_details, COUNT(state_details) cnt FROM couriers
WHERE state_details <> -1
GROUP BY state_details) a
...
SELECT hhd.creator_id, u.surname, u.name, hhd.any_actions 
FROM
	(SELECT creator_id, COUNT(*) any_actions
FROM history_details hd
JOIN 
(SELECT * FROM histories
WHERE object_type = 1 AND created >= '2021,9,1' AND created <= '2021,9,30' AND creator_id IN
	(SELECT id FROM users WHERE department_id = 6)) h
ON h.id = hd.history_id
GROUP BY creator_id) AS hhd
JOIN users u ON u.id = hhd.creator_id;
...
SELECT h.creator_id, u.surname, u.name, COUNT(*) vosstan_priglasheno FROM history_details hd
JOIN histories h ON h.id = hd.history_id
JOIN users u ON h.creator_id = u.id
WHERE ((hd.field_name = 'StateId' AND hd.value_new = 37 AND h.object_type = 3) OR (hd.field_name = 'StateDetails' AND hd.value_new = 19 AND h.object_type = 1)) AND h.created >= '2021,9,1' AND h.created <= '2021,9,30' AND h.creator_id IN
